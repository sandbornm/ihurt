...
import { blankEntry, finishEntry, stringifyNotebook, parseNotebook, exportNotebook } from '../src/notebook-data.ts';
...
test('notebook round trip preserves the original notes, pin comments and anatomy references', () => {
 ...
 entry.points![0].source={atlas:'z-anatomy-v1',layer:'muscle',asset:'muscular.glb',node_index:1,mesh_name:'Example mesh'};
...
test('activity suggestions never become reported locations', () => {
 const entry=blankEntry(); entry.map.activity='Tennis';
 ...
 assert.equal(parseNotebook(stringifyNotebook([entry]))[0].map.activity,entry.map.activity);
});
test('import rejects incompatible models, malformed points and unsafe links', () => {
 const make=()=>JSON.parse(stringifyNotebook([finishEntry(exampleEntry())]));
 for(const change of [
  ...
  (v:any)=>v.entries[0].related_reading=[{url:'http://example.com',title:'Example',publisher:'Example',checked:'2026-09-12'}],
 ]) { const value=make(); change(value); assert.throws(()=>parseNotebook(JSON.stringify(value))); }
});
test('printout includes pin commentary and escapes all supplied text', () => {
 const entry=finishEntry(exampleEntry()); entry.note='<example & note>'; entry.map.summary=entry.note;
 const svg=renderMapSvg(entry);
 assert.ok(svg.includes('&lt;example &amp; note&gt;'));
 assert.ok(svg.replace(/<[^>]+>/g, ' ').replace(/\s+/g,' ').includes('High left neck when looking down'));
